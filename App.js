      ...
      <div className="container">
        <div className="row">
          ...
          <div className="three columns expand-btn-container">
            <button className="button-primary" id="expand-btn" onClick={this.handleExpand}>Apply Filters</button>
            <button className="button-primary" id="reset-btn" onClick={this.resetCoins}>Reset Filters</button>
          </div>
          <div className="twelve columns expand " ref={(value)=>this.expand = value}>
            <form onSubmit={this.handleSubmit} >
                <label for="filter-coins">What coins do you have?</label>
                ...
                <input className="button-primary" type="submit" value="Apply Filter" ref={(value) => this.filterSbmt = value} />
            </form>
            <form onSubmit={this.handleSubmitCoinAmount} >
                <label>How many coins do you have?</label>
                <input type="text" name="coinId" value={this.state.coinId} onChange={this.handleInputChange} placeholder="Coin Symbol e.g btc,eth,ltc" required />
                <input type="number" step="any" min="0" name="coinAmount" value={this.state.coinAmount} onChange={this.handleInputChange} placeholder="Quantity of coins i.e. 123.04" required />
                <input className="button-primary" type="submit" value="Add" ref={(value) => this.amountSbmt = value} />
            </form>
          </div>
        </div>
        <img className="loading-gif" id="loading-gif" style={{display: 'block',margin: "0 auto",height: '100px',width: '500px'}} src="./loading.gif " alt="Loading..."></img>
        {this.renderSpinners()}
      </div>
    );
  }
